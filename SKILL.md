...
1. Find probeable symbols: `ctrace_symbols(filter="tick")` — returns mangled names and flags tick candidates
2. Define the loop: `ctrace_define_tick(name="physics", function="<mangled_name>")`
3. Optionally scope to one thread: `ctrace_threads()` → pick `name` → pass as `thread_filter`
4. Measure: `ctrace_tick_summary(tick_name="physics", duration=30)`
5. Find outliers: `ctrace_tick_outliers(tick_name="physics", threshold_us=1000)` — each outlier includes `syscall_count`, `alloc_count`, `alloc_bytes`
6. Compare fast vs slow: `ctrace_tick_compare(tick_name="physics")`
...
- `ctrace_tick_outliers` reports each outlier's `duration_us`, `syscall_count`, `alloc_count`, and `alloc_bytes`. For deeper per-outlier detail (which specific syscalls, stack traces), use `ctrace_probe` with a custom script.
- `ctrace_symbols` returns `mangled` names ready to pass to `ctrace_define_tick` — no need to run `nm` manually.
- `ctrace_threads` returns thread `name` values ready to pass as `thread_filter` to `ctrace_define_tick`.
